...
class PANode:  # Path Algebra Node
    ...
    def __eq__(self, other):
        ...
        if self.pop == POp.PROP:
            return self.pop == other.pop and self.children[0] == other.children[0]
...
def _parse_path(graph: Graph, path: BNode) -> PANode:
    ...
    transl = {
        SH.inversePath: POp.INV,
        SH.zeroOrMorePath: POp.KLEENE,
        SH.zeroOrOnePath: POp.ZEROORONE,
    }
    for predicate in [SH.inversePath, SH.zeroOrMorePath, SH.zeroOrOnePath]:
    ...
    if (path, SH.oneOrMorePath, None) in graph:
        ...
        return PANode(POp.COMP, [parsed_rest, PANode(POp.KLEENE, [parsed_rest])])
